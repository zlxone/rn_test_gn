/*
* @author:  Abyss
* @createTime:  2017-11-06, 09:50:08 GMT+0800
	...
	render(){
		...
		return (
			<TouchableOpacity activeOpacity={counting ? 1 : 0.8} onPress={()=>{
				...
				{/* 颜色变化修改到背景上 */}
				{/* <View style={[{width:120, height:44,justifyContent:'center',alignItems:'center'},style,{backgroundColor: ((!counting && enable && selfEnable) ? '#38A2FB' : '#DDDDDD')}]}>
					<Text style={textStyle}>{timerTitle}</Text>
				</View> */}

				{/* 颜色变化修改到背景上 */}
				{/* <View style={[{width:120, height:44,justifyContent:'center',alignItems:'center'},style,{backgroundColor: ((!counting && enable && selfEnable) ? '#46BE67' : '#EDEDED')}]}>
					<Text style={[textStyle,{color : ((!counting && enable && selfEnable) ? '#FFFFFF' : '#B7B7B8')}]}>{timerTitle}</Text>
				</View> */}

				<View style={[{width:120, height:44,justifyContent:'center',alignItems:'center'},style,{backgroundColor: '#FFFFFF'}]}>
					<Text style={[textStyle,{color : ((!counting && enable && selfEnable) ? '#F0372B' : '#B7B7B8')}]}>{timerTitle}</Text>
				</View>
			</TouchableOpacity>
		)
	}
